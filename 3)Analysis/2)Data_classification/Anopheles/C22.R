...
adjust <- 3 #frames to adjust precisely the location of the 1st trial
...
Sti_range<-Sti_range %>% 
  filter(Trial!=11) %>% 
  add_row(Trial=11,frameD=30470,frameF=(frameD+74)) %>% 
  arrange(Trial)
Display_response(Sti_range,11) #input = range + number of stimulus to plot

Sti_range<-Sti_range %>% 
  filter(Trial!=12) %>% 
  add_row(Trial=12,frameD=32665,frameF=(frameD+74)) %>% 
  arrange(Trial)
