...
Sti_range<-Sti_range %>% 
  filter(Trial!=10) %>% 
  add_row(Trial=10,frameD=27400,frameF=(frameD+74)) %>% 
  arrange(Trial)
Display_response(Sti_range,10) #input = range + number of stimulus to plot

Sti_range<-Sti_range %>% 
  filter(Trial!=11) %>% 
  add_row(Trial=11,frameD=30400,frameF=(frameD+74)) %>% 
  arrange(Trial)
Display_response(Sti_range,11) #input = range + number of stimulus to plot

Sti_range<-Sti_range %>% 
  filter(Trial!=12) %>% 
  add_row(Trial=12,frameD=33400,frameF=(frameD+74)) %>% 
  arrange(Trial)
Display_response(Sti_range,12) #input = range + number of stimulus to plot
