...
Sti_range<-Sti_range %>% 
  filter(Trial!=1) %>% 
  add_row(Trial=1,frameD=500,frameF=(frameD+74)) %>% 
  arrange(Trial)
Display_response(Sti_range,1) #input = range + number of stimulus to plot
Sti_range<-Sti_range %>% 
  filter(Trial!=10) %>% 
  add_row(Trial=10,frameD=28090,frameF=(frameD+74)) %>% 
  arrange(Trial)
Display_response(Sti_range,10) #input = range + number of stimulus to plot

Sti_range<-Sti_range %>% 
  filter(Trial!=12) %>% 
  add_row(Trial=12,frameD=34260,frameF=(frameD+74)) %>% 
  arrange(Trial)
