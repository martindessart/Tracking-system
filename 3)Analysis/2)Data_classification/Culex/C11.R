...
filename="R11"
trial_nb <- 12 #number of trials -> 10 for group Expe & Ctrl.1, 1 for Ctrl.2
...
#Special case HERE : TWO VIDEOS COMBINED HAD SHIFTED THE INTERTRIAL INTERVAL
# --> Manually shift ranges starting from the 5th trial:
...
Sti_range<-Sti_range %>% 
  ...
  filter(Trial!=11) %>% 
  filter(Trial!=12) %>% 
  add_row(Trial=4,frameD=10400,frameF=(frameD+74)) %>% 
  add_row(Trial=5,frameD=13300,frameF=(frameD+74)) %>% 
  add_row(Trial=6,frameD=16250,frameF=(frameD+74)) %>% 
  add_row(Trial=7,frameD=19170,frameF=(frameD+74)) %>% 
  add_row(Trial=8,frameD=22300,frameF=(frameD+74)) %>% 
  add_row(Trial=9,frameD=25260,frameF=(frameD+74)) %>% 
  add_row(Trial=10,frameD=28480,frameF=(frameD+74)) %>% 
  add_row(Trial=11,frameD=30120,frameF=(frameD+74)) %>% 
  add_row(Trial=12,frameD=32780,frameF=(frameD+74)) %>% 
  arrange(Trial)

Display_response(Sti_range,4) #input = Df + trial number to plot
Display_response(Sti_range,5) #input = Df + trial number to plot
Display_response(Sti_range,6) #input = Df + trial number to plot
Display_response(Sti_range,7) #input = Df + trial number to plot
Display_response(Sti_range,8) #input = Df + trial number to plot
Display_response(Sti_range,9) #input = Df + trial number to plot
